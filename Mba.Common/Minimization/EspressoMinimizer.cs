...
{
    public struct OnOffSet
    {
        public readonly ulong DemandedMask = 0;

        public readonly ulong DontCareMask = 0;

        public ulong NegatedMask => ~DemandedMask & DontCareMask;

        public OnOffSet(ulong demandedMask, ulong dontCareMask)
        {
            DemandedMask = demandedMask;
            DontCareMask = dontCareMask;
        }
    }

    public static class EspressoMinimizer
    {
        public static (AstNode ast, List<OnOffSet> onOffs) SimplifyBoolean(List<int> resultVector, IReadOnlyList<VarNode> variables)
        {
            // Run espresso.
            ...
            return ParsePLA(output, resultVector, variables);
        }

        // Generate an instance of Espresso's file format.
        // https://user.engineering.uiowa.edu/~switchin/OldSwitching/espresso.5.html
        public static string GeneratePLA(List<int> resultVector, IReadOnlyList<VarNode> variables)
        {
        ...
        private static (AstNode ast, List<OnOffSet> onOffs) ParsePLA(string output, List<int> resultVector, IReadOnlyList<VarNode> variables)
        {
            ...
            {
                var andClauses = new List<AstNode>();
                ...
                for (int i = 0; i < variables.Count; i++)
                {
                    ...
                    else if (c == '0')
                    {
                        ...
                        andClauses.Add(new NegNode(variables[i]));
                    }
            ...
            if (!terms.Any())
            {
                ...
                var constant = new ConstNode((ulong)0, variables[0].BitSize);
